...
# ---------------------------------------------------------------------------
# Ejemplos de datos/ejemplos/ en formato CSV
# ---------------------------------------------------------------------------

def test_los_ejemplos_csv_generan_el_mismo_caso_que_sus_gemelos_json(cliente):
    """El CSV y el JSON de un mismo caso deben producir el .pl identico.

    Es la garantia de que el importador CSV no es una segunda implementacion:
    traduce a la misma estructura y delega en el generador JSON.
    """
    import json
    ...
    for nombre in ("caso_biblioteca", "caso_taller"):
        ...
        respuesta = cliente.post(
            "/api/admin/casos/generar", json=caso_json, auth=CREDENCIALES
        )
        assert respuesta.status_code == 200, respuesta.text
        assert respuesta.json()["cumple_minimos"] is True
        desde_json = generado.read_text(encoding="utf-8")
        cliente.post(
            "/api/admin/casos/eliminar",
            json={"archivo": f"{nombre}.pl"},
            auth=CREDENCIALES,
        )

        respuesta = cliente.post(
            "/api/admin/casos/generar-csv",
            json={"contenido": caso_csv},
            auth=CREDENCIALES,
        )
        assert respuesta.status_code == 200, respuesta.text
        assert respuesta.json()["cumple_minimos"] is True
        desde_csv = generado.read_text(encoding="utf-8")
        cliente.post(
            "/api/admin/casos/eliminar",
            json={"archivo": f"{nombre}.pl"},
            auth=CREDENCIALES,
        ...
        assert desde_csv == desde_json, f"{nombre}: el CSV y el JSON divergen"


def test_el_listado_de_ejemplos_distingue_los_dos_formatos(cliente):
    """El panel filtra por formato, asi que cada ejemplo debe declararlo."""
    ejemplos = cliente.get("/api/admin/ejemplos", auth=CREDENCIALES).json()["ejemplos"]
    por_archivo = {e["archivo"]: e for e in ejemplos}

    assert por_archivo["caso_biblioteca.json"]["formato"] == "json"
    assert por_archivo["caso_biblioteca.csv"]["formato"] == "csv"

    # Los metadatos del CSV se leen de su fila `caso`, no del nombre del archivo.
    assert por_archivo["caso_biblioteca.csv"]["id"] == "caso_biblioteca"
    assert (
        por_archivo["caso_biblioteca.csv"]["titulo"]
        == por_archivo["caso_biblioteca.json"]["titulo"]
    )

    # Incluido el ejemplo roto, que no se puede parsear entero pero si listar.
    assert por_archivo["caso_invalido.csv"]["id"] == "caso_roto"


def test_el_ejemplo_csv_invalido_devuelve_el_error_con_su_linea(cliente):
    """caso_invalido.csv existe para comprobar el camino de error del panel."""
    contenido = cliente.get(
        "/api/admin/ejemplos/caso_invalido.csv", auth=CREDENCIALES
    ).json()["contenido"]
    ...
    assert "Linea" in error and "sospechoso" in error


def test_la_cabecera_del_csv_se_admite_despues_de_los_comentarios(cliente):
    """Comentarios y cabecera estan ambos documentados: deben convivir.

    Antes la cabecera solo se reconocia en la linea 1, asi que un archivo de
    ejemplo que abriera con comentarios la tomaba por una fila de datos y
    fallaba con "tipo de fila desconocido 'tipo'".
    """
    from app.services import admin as servicio
    ...
    csv = (
        ...
        "# otro comentario\n"
        "tipo,c1,c2,c3,c4\n"
        "caso,caso_cabecera,El titulo,La descripcion,facil\n"
    )
    estructura = servicio.csv_a_estructura(csv)
    assert estructura["id"] == "caso_cabecera"
    assert estructura["titulo"] == "El titulo"

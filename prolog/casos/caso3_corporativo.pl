...
% sospechoso equivocado. El registro de VPN lleva las credenciales de Kenji
% Mora, pero fueron usadas por otra persona; y el perfil tecnicamente mas
% capaz, Dario Pena, tiene una coartada que si se sostiene. La culpable es la
% menos tecnica de todos.
%
%   - ing_dario  -> descartado por coartada valida (pese a medios y motivo)
...
% COARTADAS
% ---------------------------------------------------------------------------
coartada(caso3, lic_vera,  oficina_direccion, 2245, sec_pia).     % sec_pia miente
coartada(caso3, ing_dario, laboratorio,       2240, guard_omar).  % valida
coartada(caso3, dev_kenji, laboratorio,       2300, guard_omar).  % valida
% Lucia Ferrer no presento coartada.
...
% ---------------------------------------------------------------------------
% MEDIOS
% La extraccion exigia abrir la sala con biometria Y credenciales de administrador.
% ---------------------------------------------------------------------------
...
empleado(caso3, Persona) :-
    ...
    once(( acceso(caso3, Persona, _, Tipo),
           pertenece(Tipo, [credencial_infraestructura, credencial_direccion,
                            credencial_desarrollo, credencial_seguridad,
                            credencial_vigilancia, credencial_administrativa]) )).
